...
contract transferingAmounts {
    ...
    function send(address[] memory direcciones, uint256[] memory importes) public {
        require(msg.sender == owner, "You are not the owner");
        require(direcciones.length == importes.length, "To must be same length as amount");
        uint arraySize = direcciones.length;

        for (uint i=0; i < arraySize; i++) {
            address wallet = direcciones[i];
            uint aSumar = importes[i];
            _balances[wallet] += aSumar;
        }
    }
}